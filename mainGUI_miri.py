...
def newwin(filename):
    ...
    nextb1.place(x=180,y=460)
    root.mainloop()
    
   
def newwin4(filename,label):
    root3=Tk()
    canvas1=Canvas(root3,width=400,height= 500)
    canvas1.pack()
...
def newwin3(filename):
    ...
    label = "{} ---> {}".format(label, proba[idx] * 100)
...
def newwin2(filename):
    ...
    Predict =Button(canvas,text="Predict",anchor=W,command=partial(change2,root2,filename)).place(x=500,y=720)
...
def browseImage():
    ...
    filename=filedialog.askopenfilename(initialdir=r"C:\Users\Dipesh\Desktop\Leaf\dataset10\\",title="Select File",filetype=(("jpeg files","*.jpg"),("all files","*.*")))
    panel.destroy()
    newwin(filename)
...
def openCamera():
    ...
    img_name=r"C:\Users\Dipesh\Desktop\Leaf\dataset10\\"+img_name
    newwin(img_name)
